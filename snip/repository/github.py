...
class GithubApi:
    ...
    def repository_content(owner, repository, path):
        url = "https://api.github.com/repos/{}/{}/contents/{}".format(owner, repository, path)
        return requests.get(url)
        ...
        if not self._root:
            ...
            for child in self._root._children:
                if child.path == snippet_id:
                    response = GithubApi.repository_content(self.owner, self.repository, child.path)
                    content_response = json.loads(response.content.decode(response.encoding))
    ...
    def _recursive_path_explorer(self, root, owner, repository, tree_hash, recursive):
        ...
        for child in current_tree:
            ...
            if child_type == "tree" and current_tree_truncated:
                ...
                self._recursive_path_explorer(subtree, owner, repository, child["sha"], "1")
